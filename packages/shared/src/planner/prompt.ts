...
export function buildPlannerPrompt(args: PlannerPromptArgs): {
  ...
  const system = [
    ...
    '- for a `"subject"` ONLY, add `categoryHints` — the vocabulary that tells this',
    "  kind of place from another, as it would appear in a listing's category field",
    "  (Google Maps writes \"Mexican restaurant\"; OpenStreetMap writes cuisine tags",
    '  like "mexican", "arepa;venezuelan", "chocolate;crepe;dessert"):',
    '  * `strong` — categories that ARE this kind of place ("mexican", "taqueria",',
    '    "tex mex", "burrito");',
    '  * `related` — adjacent, but not what was asked for ("venezuelan", "arepa",',
    '    "latin american", "peruvian", "spanish");',
    '  * `excluded` — a different kind of place that a search for this one turns up',
    '    anyway ("dessert", "chocolate", "crepe", "bakery", "cafe", "ice cream");',
    "  give a handful of each, lowercase, in the SEARCH LANGUAGE and in English —",
    "  category fields are rarely translated. These are matched as whole words, so",
    '  write both "arepa" and "arepas" if both are plausible;',
